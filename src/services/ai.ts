...
function formatEntriesForAnalysis(entries: DayEntry[]): string {
  ...
  return entries.map(entry => {
    ...
    if (entry.morning) {
      ...
      text += `  Qualidade: ${SLEEP_QUALITY_LABELS[m.sleepQuality]}\n`;
      text += `  Despertares: ${m.nightAwakenings}\n`;
      text += `  Janta: ${MEAL_SIZE_LABELS[m.dinner]}\n`;
      text += `  Sono ao acordar: ${WAKE_SLEEPINESS_LABELS[m.wakeSleepiness]}\n`;
    ...
    if (entry.noon) {
      const n = entry.noon;
      text += `Meio-dia:\n`;
      text += `  Sono de manhã: ${SLEEPINESS_LEVEL_LABELS[n.morningSleepiness]}\n`;
      if (n.sleepinessTime) text += `  Horário: ${n.sleepinessTime}\n`;
      text += `  Dormiu: ${n.slept ? "Sim" : "Não"}\n`;
      text += `  Luz solar: ${n.sunlight ? "Sim" : "Não"}\n`;
      text += `  Café da manhã: ${MEAL_SIZE_LABELS[n.breakfast]}\n`;
      text += `  Café: ${n.coffee ? "Sim" : "Não"}\n`;
      text += `  Doce: ${n.sweets ? "Sim" : "Não"}\n`;
      text += `  Exercício: ${n.exercise ? "Sim" : "Não"}\n`;
      text += `  Foco: ${FOCUS_LEVEL_LABELS[n.focus]}\n`;
      text += `  Estresse: ${STRESS_LEVEL_LABELS[n.stress]}\n`;
      text += `  Ansiedade: ${ANXIETY_LEVEL_LABELS[n.anxiety]}\n`;
    ...
    if (entry.evening) {
      ...
      text += `Noite:\n`;
      text += `  Sono à tarde: ${SLEEPINESS_LEVEL_LABELS[e.afternoonSleepiness]}\n`;
      if (e.sleepinessTime) text += `  Horário: ${e.sleepinessTime}\n`;
      text += `  Dormiu: ${e.slept ? "Sim" : "Não"}\n`;
      text += `  Almoço: ${MEAL_SIZE_LABELS[e.lunch]}\n`;
      text += `  Café: ${e.coffee ? "Sim" : "Não"}\n`;
      text += `  Doce: ${e.sweets ? "Sim" : "Não"}\n`;
      text += `  Exercício: ${e.exercise ? "Sim" : "Não"}\n`;
      text += `  Foco: ${FOCUS_LEVEL_LABELS[e.focus]}\n`;
      text += `  Estresse: ${STRESS_LEVEL_LABELS[e.stress]}\n`;
      text += `  Ansiedade: ${ANXIETY_LEVEL_LABELS[e.anxiety]}\n`;
