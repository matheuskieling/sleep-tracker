...
import { submitEveningEntry, clearFormSection } from "../../src/services/firestore";
...
export default function EveningFormScreen() {
  ...
  const handleClear = async () => {
    try {
      await clearFormSection(user!.uid, date, "evening");
    ...
    <View className="flex-1 bg-surface">
      ...
      {loading ? (
        ...
        <EveningForm
          ...
          initialData={entry?.evening}
          onClear={entry?.evening ? handleClear : undefined}
        />
      )}
    </View>
  );
